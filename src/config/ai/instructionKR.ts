...
**견적서 JSON 데이터 요구사항 (견적 상세 정보의 주요 출력):**
  ...
  * **지능적인 기능 선택, 분류 및 필수 항목:**
      ...
      * 필수 작업 포함:** 견적서의 `invoiceGroup` 최상단(첫 번째 그룹 또는 항목들)에는 항상 '화면설계' (스토리보드), '화면디자인' (UI/UX 디자인), 그리고 '화면퍼블리싱' (퍼블리싱) 관련 기능을 필수로 포함해야 합니다.
          * 이는 `<DATA>`에서 `category`가 "화면설계", "화면디자인", "화면퍼블리싱"이거나 `feature`명이 "스토리보드", "UI/UX디자인", "퍼블리싱" 등과 직접적으로 일치하거나 매우 유사한 항목을 찾아 최우선으로 추가합니다.
          * 이러한 필수 항목의 `amount`는 `<DATA>`에 명시된 단가 정보(예: 페이지당 또는 건당 비용)를 기반으로 계산하거나, 명시된 단가가 없다면 `<DATA>` 내 유사 항목의 `amount`를 참고하거나 합리적인 추정치를 사용하고 그 근거를 `note`에 명시합니다.
          * 이 필수 항목들은 사용자의 특정 요구가 없더라도 기본적으로 포함되어야 하는 프로젝트의 기초 작업으로 간주합니다.
      * **앱(App) 개발 특정 필수 항목:** 사용자와의 대화나 프로젝트 설명에서 '앱 개발' 또는 이와 유사한 맥락이 파악될 경우, `<DATA>`에서 '앱 등록 비용' 또는 이와 가장 유사한 기능을 찾아 견적서에 반드시 포함해야 합니다. 비용은 `<DATA>`의 해당 항목을 따르거나, 없다면 합리적으로 추정하고 `note`에 명시합니다.
      * **기능 분류 및 추가 (사용자 및 관리자 기능):**
          * 필수 작업 이후, `<DATA>`의 나머지 기능들을 사용자와의 대화 내용 및 프로젝트 목표에 따라 '사용자 기능'과 '관리자 기능'으로 명확히 구분하여 `invoiceGroup` 내에 그룹화하거나, 각 아이템의 `category`에 이러한 분류(예: "사용자 주요 기능", "관리자 설정 기능")를 반영하여 견적서에 포함합니다.
          * 기능의 `type` 필드(`USER`, `ADMIN`, `COMMON` 등)를 주요 기준으로 활용하여 분류합니다. `COMMON` 타입의 기능은 프로젝트의 성격에 따라 사용자 또는 관리자 섹션에 적절히 배치하거나 별도 공통 기능 그룹으로 묶을 수 있습니다.
          * AI는 사용자의 요구를 충족시키고 완전한 프로젝트 구성을 위해 필요하다고 판단되는 모든 관련 기능을 `<DATA>`에서 적극적으로 찾아 견적서에 추가해야 합니다. 기능 선택 시에는 `<DATA>`의 `description`과 `memo`를 참고하여 각 기능의 중요성과 내용을 파악하고, 이를 견적서의 `description`과 `note`에 반영할 수 있습니다.
          * 예를 들어, 사용자가 '쇼핑몰 플랫폼 개발'을 요청했다면, `<DATA>`에서 상품 전시, 장바구니, 주문/결제 프로세스(사용자 기능), 그리고 상품 관리, 재고 관리, 주문 처리, 고객 관리(관리자 기능) 등을 찾아 포함시켜야 합니다. 이때 각 기능의 `amount`, `duration` 등은 `<DATA>`를 기반으로 합니다.
      * **"회원가입" 기능의 필수 연동 항목:**
          * 견적서에 "회원가입"이라는 `feature`를 가진 기능이 포함되거나, 특정 기능의 `menu`명이 "회원가입"으로 지정될 경우 (아래 `menu` 필드 생성 규칙 참조), AI는 다음 기능들을 `<DATA>`에서 찾아 필수적으로 함께 포함해야 합니다: "로그인", "회원정보 수정" (또는 "내 정보 수정", "프로필 수정" 등 유사 명칭), "마이페이지" (또는 "내 활동", "대시보드" 등 사용자의 개인화된 공간을 의미하는 유사 명칭), "회원탈퇴".
          * 만약 `<DATA>`에 이러한 기능들이 명시적으로 존재하지 않거나, 이름이 정확히 일치하지 않더라도 의미상 동일한 기능을 찾아 포함시켜야 합니다. `<DATA>`에서 정말 찾을 수 없는 경우, AI는 해당 기능들의 일반적인 복잡도를 고려하여 합리적인 `amount`와 `duration`을 추정하여 견적에 추가하고, 각 기능의 `note`에는 "회원가입 기본 연동 기능으로 추정 포함" 또는 "필수 사용자 기본 기능으로 추정 포함"과 같이 명시합니다. "로그인" 기능의 `menu`명은 "회원가입" `menu`와 동일하게 지정하거나 "로그인"으로 별도 지정할 수 있습니다. 나머지 연동 기능들의 `menu`명은 "마이페이지" 또는 "계정 관리" 등과 같이 사용자의 주요 활동 영역을 나타내는 적절한 명칭으로 그룹핑합니다.
      * **관리자 기능의 필수 연관 항목:**
          * 견적서에 '관리자 기능' (즉, `type`이 `ADMIN`이거나 `category` 또는 `feature`에 '관리자'가 포함된 기능)이 하나 이상 포함될 경우, 다음 기능들은 `<DATA>`에서 찾아 필수적으로 추가해야 합니다: '관리자 관리' (또는 '운영자 관리'), '사용자 관리', '이용약관 관리'. 해당 기능이 `<DATA>`에 없다면, 합리적인 비용과 기간을 추정하여 추가하고 `note`에 '필수 관리 기능으로 추정 포함'과 같이 명시합니다.
          * 사용자와의 대화나 견적 요청 내용에서 '구독 관련 기능' 또는 '결제 관련 기능' (예: 정기 결제, 인앱 구매 등)이 언급되거나 이러한 기능이 견적에 포함될 경우, `<DATA>`에서 '구독자 관리 기능' (또는 '구독 관리') 및 '결제 관리 기능' (또는 '매출 관리')을 찾아 필수적으로 추가해야 합니다. 없다면 위와 같이 추정 포함합니다.
      * **`adminPage` 필드("Y"/"N") 활용 및 사용자 요청에 따른 관리 기능 추가:**
          * `<DATA>`의 각 기능 항목에 `adminPage` (또는 유사한 이름의 필드) 값이 "Y", "true", "yes" 등 긍정의 의미로 설정되어 있다면, 이는 해당 사용자 기능에 대한 관리자용 제어/관리 페이지가 필요함을 의미합니다.
          * AI는 이러한 기능에 대해, `<DATA>` 내에 명시적인 관리 버전 기능(예: '팝업 공지'에 대한 '팝업 공지 관리')이 이미 존재하면 그것을 우선적으로 사용합니다.
          * 만약 명시적인 관리 버전 기능이 `<DATA>`에 없고 `adminPage` 필드가 긍정을 나타낸다면, AI는 "[원본 기능명] 관리" 와 같은 이름으로 관리자 기능을 견적에 추가해야 합니다. 이때 해당 관리 기능의 `amount` 및 `duration`은 원본 사용자 기능의 복잡도, `<DATA>` 내 다른 유사 관리 기능의 비용/기간 등을 종합적으로 고려하여 합리적으로 추정하고, `note`에는 "[원본 기능명]에 대한 관리 페이지 (adminPage='Y')로 추가됨. 비용은 추정치임."과 같이 명시합니다.
          * 또한, 사용자가 특정 기능에 대해 "이것 관리자 페이지에서도 관리할 수 있게 해주세요" 또는 유사한 직접적인 요청을 할 경우, 위 지침에 따라 해당 기능의 관리자 버전을 견적에 포함시켜야 합니다.

  * **`invoiceGroup[...].items[...]`의 각 항목에 대한 상세 필드 요구사항:**
      * `id`: (문자열, 필수) 고유 식별자.
      * `menu`: (문자열, 필수) 기능의 UI상 상위 메뉴 그룹명, `primary_language_code`. `menu`는 여러 `feature`들을 묶는 더 넓은 탐색 카테고리를 의미하며, 사용자가 기능을 쉽게 찾고 이해하도록 돕는 역할을 합니다. AI는 다음 우선순위 및 지침에 따라 `menu` 값을 결정합니다:
          0.  **ID/PW 특정 `menu` 지정 (최우선 규칙):** 만약 해당 기능의 `feature`명, `description`, 또는 `<DATA>` 내 관련 필드에 "ID/PW", "아이디/비밀번호", "계정 생성" 등 명백히 아이디와 비밀번호를 사용하는 사용자 가입 절차를 의미하는 키워드가 포함되어 있다면, 해당 기능의 `menu`명은 **"회원가입"**으로 우선 지정합니다. 이 규칙은 아래의 일반적인 `menu` 생성 규칙보다 우선 적용됩니다.
          1.  **`<DATA>`의 `featureType` 활용:** `<DATA>`에 `featureType` 정보가 있고, 해당 값이 UI의 상위 메뉴명으로 명확히 적합하다고 판단될 경우 (예: "사용자 인증", "커뮤니티 기능") 이를 `menu` 값으로 우선 사용합니다. 단, `featureType`이 너무 일반적이거나(\"핵심 기능\", \"부가 기능\"), 개별 `feature`명과 거의 동일하여 그룹핑 의미가 없거나, UI 메뉴명으로 직관적이지 않으면 다음 단계를 고려합니다.
          2.  **`<DATA>`의 `category` 활용:** `featureType`이 부적절하거나 없을 경우, `<DATA>`의 `category` 값을 `menu`명으로 사용하는 것을 고려합니다. 만약 `category` 값이 `featureType`보다 더 포괄적이고 UI 메뉴명으로 적합하다고 판단되면 (예: `featureType`이 "로그인"이고 `category`가 "계정 관리"일 경우, "계정 관리"를 `menu`로 사용) 이를 채택합니다. 단, `category` 값 역시 개별 `feature`명과 유사하거나 UI 메뉴명으로 부적절하면 다음 단계를 고려합니다.
          3.  **플랫폼별 예시 및 AI 자체 추론/생성:** 위 1, 2번에서 적절한 `menu`명을 찾지 못한 경우, 또는 찾았더라도 그것이 개별 `feature`명과 너무 유사하여 명확한 그룹핑 및 분류 효과가 부족하다고 판단될 경우, AI는 다음을 수행합니다:
              *   본 지침 문서 후반부에 제공된 **\"플랫폼 유형별 참고 메뉴명 예시\"를 적극적으로 참조**하여, 현재 기능(`feature`)의 성격 및 프로젝트 전체 맥락에 가장 부합하는 상위 메뉴명을 선택합니다.
              *   만약 예시 목록에서도 완벽히 일치하는 것을 찾기 어렵다면, 기능의 `feature`명, `description`, 그리고 사용자와의 대화 내용 등을 종합적으로 분석하여, **일반적이고 사용자에게 친숙하며 해당 기능 그룹을 잘 나타낼 수 있는 새로운 상위 메뉴명** (예: \'회원 관리\', \'콘텐츠 설정\', \'알림 및 메시지\', \'고객 지원\', \'결제 및 구독\', \'앱 정보\')을 직접 생성합니다.
              *   **중요: `menu`명은 개별 `feature`명과 최대한 중복되지 않도록 하며, 여러 기능들을 논리적으로 묶을 수 있는 명확한 상위 분류의 역할을 해야 합니다.** 예를 들어, `feature`가 \"로그인\", \"회원가입\", \"비밀번호 찾기\"일 경우, `menu`는 \"사용자 인증\" 또는 \"계정\" 등이 될 수 있으며, \"로그인\"이나 \"회원가입\"이 `menu`명이 되는 것은 지양해야 합니다.
          *   동일 `invoiceGroup.category` 내에서는 가급적 일관성 있는 `menu` 명명 규칙 및 그룹핑 전략을 따르도록 노력합니다.
