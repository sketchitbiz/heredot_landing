...
function DataContainer({ message, successChild, noDataChild }: DataContainerProps) {
  ...
  return <div style={{ flex: 1 }}>{isSuccess ? successChild : noDataChild ?? <p>No data available.</p>}</div>;
}
...
function ProductDetailContent({ title, data, color }: ProductDetailContentProps) {
  return (
    <div style={{ backgroundColor: color, padding: "1rem", borderRadius: "8px" }}>
      <h3>{title}</h3>
      <pre style={{ fontSize: "13px", overflowX: "auto" }}>{JSON.stringify(data, null, 2)}</pre>
    </div>
  );
}
      ...
      <main style={{ paddingTop: "2rem", paddingBottom: "2rem" }}>
        ...
        <section style={{ display: "flex", gap: "2rem", marginTop: "2rem" }}>
          <DataContainer
            ...
            successChild={
              <ProductDetailContent title="✅ 구독자 있음 (productIndex: 25)" data={productDetail1} color="#e0f7fa" />
            }
            noDataChild={
              <ProductDetailContent title="❌ 구독자 없음 (productIndex: 25)" data={productDetail1} color="#ffe0b2" />
            }
          />
